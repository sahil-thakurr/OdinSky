...
const Header = () => {
  ...
  return (
    ...
    <div className="container mx-auto flex h-16 items-center justify-between px-4">
          ...
          <Link to={"/"}>
          <img src={theme==='dark' ? "/logo_left.png" : "/logo_right.png"} alt="Weather-icon" 
          className="h-30"/>
          </Link>
          <div className="flex gap-4">
            ...
            <div onClick={()=>setTheme(isDark? "light" : "dark") } className={`flex items-center cursor-pointer transition-transform duration-500 ${isDark?"rotate-180" : "rotate-0"}`}>
              {isDark? <Sun className="h-6 w-6 text-yellow-500 rotate-0 transition-all"/> : <Moon className="h-6 w-6 text-yellow-500 rotate-0 transition-all"/>}
            </div>
        </div>
    </div>
    </header>
